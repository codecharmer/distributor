...
jQuery( window ).on( 'load', () => {
	...
	function distributorMenuEntered() {
		...
		jQuery('#bulk-edit').find("#bulk-titles button").each(function(){
			$post_ids.push(jQuery(this).attr('id').substring(1))
			});
			console.log($post_ids);
			if($post_ids.length>0)
			dt.postId=$post_ids[0];
	
			const data = {
				action: 'dt_load_connections',
				loadConnectionsNonce: dt.loadConnectionsNonce,
				postId: dt.postId,
				posts_array:$post_ids
			};
